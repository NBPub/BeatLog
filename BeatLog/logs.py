...
def add_regex_method():
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur: 
            ...
            regex_groups, regex_patterns, regex_logs= gather_RegexMethods(conn,cur)
...
def edit_log_regex(log_file):
    ...
    return render_template(f'{template}.html', method_info=method_info, alias = alias,
                            log_file=log_file, regex_methods=regex_methods, scheme=scheme,
                            alert=alert, log_methods=log_methods, test_results=test_results, time_warning=time_warning)