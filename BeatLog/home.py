...
def home():
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            ...
            # update log data - garbage collect           
            elif request.method == 'POST' and 'vacuum' in request.form:
    ...
    return render_template('home.html', homeIP=homeIP, duration=duration, logs=logs,
                            check=datetime(1,1,1), ignoreIPs=ig, alert=alert,
                            log_info=log_info)

@bp.route("/home/data_cleaning/", methods = ['GET','POST'])
def data_clean():
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:
            ...
            logs, table = log_data_cleaning(conn,cur) # logs with > 0 rows, table of data
            noIPgeo = geo_noIP_check(conn,cur)
            ...
            if request.method == 'POST' and 'Estimate' in request.form or 'confirm_delete' in request.form:
                ...
                if 'Estimate' in request.form:
                    estimate = log_clean_estimate(conn,cur, existing)                   
                    if estimate:
...
@bp.route("/home/failed_regex/", methods = ['GET','POST'])
def failed_regex():
    with pool.connection() as conn:
        with conn.cursor() as cur:
...
@bp.route("/home/settings/", methods = ['GET','POST'])
def settings():  
    alert = mm_check = None
    with pool.connection() as conn:
        with conn.cursor() as cur:     
...
@bp.route("/home/Beat/", methods = ['POST'])
def Beat():
    with pool.connection() as conn:
        with conn.cursor() as cur:    
            beatIP = request.form['BeatLog']
            beatIP = beat_analyze(conn, cur, beatIP)
    return render_template('BeatLog.html', beatIP=beatIP)

@bp.route("/home/report/", methods = ['GET', 'POST'])
def recent_report():
    ...
    with pool.connection() as conn:
        with conn.cursor() as cur:    
            ...
            f2b_unused, f2brecent = report_build(conn, cur, time_select, report_days)           
...
@bp.route("/home/jail/", methods = ['GET','POST'])
def configure_jail():
    location = None
    message = None
    made = False
        ...
        with conn.cursor() as cur:   
            ...
            if request.method == 'POST' and 'all_activity' in request.form:
                for log in jail[3]['enabled']:
                    stats = filter_info(conn, cur, log['name'])
                    log['stats'] = stats
                del stats
